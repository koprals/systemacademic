...
<div class="widget">
	<div class="title">
		...
		<h6>
			Kelas - Page <?php echo $this->Paginator->counter(); ?>
		</h6>
	</div>
	<div class="title">
	...
	<table cellpadding="0" cellspacing="0" width="100%" class="sTable mTable">
		<thead>
			<tr>
				...
				<td style="width:15%;">
					<?php echo $this->Paginator->sort("$ModelName.kd_kelas",'Kode Kelas');?>
				</td>
				<td style="width:15%;">
					<?php echo $this->Paginator->sort("$ModelName.nm_kelas",'Nama Kelas');?>
				</td>
				<td>
					<?php echo $this->Paginator->sort("$ModelName.pro_keahlian",'Program Keahlian');?>
				</td>
				<td style="width:15%;">
					<?php echo $this->Paginator->sort("$ModelName.jumlah",'Jumlah Siswa');?>
				</td>
				<td>
					<?php echo $this->Paginator->sort("$ModelName.nip",'Wali Kelas');?>
				</td>
				<?php
				if(
					$access[$aco_id]["_update"] == 1 or
			...
			<?php foreach($data as $data): ?>
				...
				<tr <?php echo $class?>>
					...
					<td style="text-align:center;">
						<input type="checkbox" id="chck_<?php echo $data[$ModelName]['kd_kelas']?>" value="<?php echo $data[$ModelName]['kd_kelas']?>"/>
					</td>
					<?php endif;?>
					<td>
						<?php echo $no ?>
					</td>
					<td><?php echo $data[$ModelName]['kd_kelas'] ?></td>
					<td><?php echo $data[$ModelName]['nm_kelas'] ?></td>
					<td><?php echo $data[$ModelName]['pro_keahlian'] ?></td>
					<td><?php echo $data[$ModelName]['jumlah'] ?></td>
					<td><?php echo $data[$ModelName]['nip'] ?></td>
					<?php
					if(
						$access[$aco_id]["_update"] == 1 or
						$access[$aco_id]["_delete"] == 1
					...
					<td style="text-align:center;">
						<?php if($access[$aco_id]["_update"] == 1):?>
							<a href="<?php echo $settings['cms_url'].$ControllerName?>/Edit/<?php echo $data[$ModelName]["kd_kelas"]?>/<?php echo $page?>/<?php echo $viewpage?>" class="tipS smallButton blueB" title="Edit">
								<img src="<?php echo $this->webroot?>img/icons/topnav/pencil.png" alt="Edit" />
							</a>
						<?php endif;?>
						<?php if($access[$aco_id]["_delete"] == 1):?>
							<a href="javascript:void(0);" onclick="Delete('Do you realy want to delete this item?','<?php echo $data[$ModelName]['kd_kelas']?>')" class="tipS smallButton redB" title="Delete">
