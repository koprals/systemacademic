...
<div class="widget">
	<div class="title">
		...
		<h6>
			News - Page <?php echo $this->Paginator->counter(); ?>
		</h6>
	</div>
	<div class="title">
	...
	<table cellpadding="0" cellspacing="0" width="100%" class="sTable mTable">
		<thead>
			<tr>
				...
				<td style="width:15%;">
					<?php echo $this->Paginator->sort("WebPageType.name",'Page Type');?>
				</td>
				<td style="width:15%;">
					<?php echo $this->Paginator->sort("$ModelName.sub_title_1",'Sub Title 1 ');?>
				</td>
				<td style="width:15%;">
					<?php echo $this->Paginator->sort("$ModelName.sub_title_2",'Sub Title 2');?>
				</td>
				<td style="width:15%;">
					<?php echo $this->Paginator->sort("$ModelName.sort",'Sort');?>
				</td>
				<td style="width:15%;">
					<?php echo $this->Paginator->sort("$ModelName.status",'Status');?>
				</td>
				<?php
				if(
					$access[$aco_id]["_update"] == 1 or
			...
			<?php foreach($data as $data): ?>
				...
				<tr <?php echo $class?>>
					...
					<td>
						...
						<a rel="lightbox" title="<?php echo $data[$ModelName]['title'] ?>" href="<?php echo $data["ImageBig"]["host"].$data["ImageBig"]["url"]?>?time=<?php echo time()?>" style="border:0px;">
						<img src="<?php echo $data["ImageThumb"]["host"].$data["ImageThumb"]["url"]?>?time=<?php echo time()?>" width="100"/>
						</a>

					</td>
					<td><?php echo chunk_split($data[$ModelName]['title'],20,"<br/>") ?></td>
					<td><?php echo chunk_split($data['WebPageType']['name'],20,"<br/>") ?></td>
					<td><?php echo chunk_split($data[$ModelName]['sub_title_1'],20,"<br/>") ?></td>
					<td><?php echo chunk_split($data[$ModelName]['sub_title_2'],20,"<br/>") ?></td>
					<td><?php echo $data[$ModelName]['sort'] ?></td>
					...
					<td style="text-align:center;">
						<?php if($access[$aco_id]["_update"] == 1):?>
							...
							<?php if($data[$ModelName]['status']=="1"):?>
								<a href="javascript:void(0);" onclick="ChangeStatus('Do you realy want hide this sponsored ?','<?php echo $data[$ModelName]['id']?>','0')" class="tipS smallButton greenB" title="Hide">
									<img src="<?php echo $this->webroot?>img/icons/topnav/arrowDown.png" alt="Hide"/>
								</a>
							<?php else:?>
								<a href="javascript:void(0);" onclick="ChangeStatus('Do you realy want publish this news ?','<?php echo $data[$ModelName]['id']?>','1')" class="tipS smallButton blackB" title="Publish">
									<img src="<?php echo $this->webroot?>img/icons/topnav/arrowUp.png" alt="Publish"/>
								</a>
							<?php endif;?>
						<?php endif;?>
						<?php if($access[$aco_id]["_delete"] == 1):?>
							<!--a href="javascript:void(0);" onclick="Delete('Do you realy want to delete this sponsored?','<?php echo $data[$ModelName]['id']?>')" class="tipS smallButton redB" title="Delete">
