...
<div class="widget">
	<div class="title">
		...
		<h6>
			Surat - Page <?php echo $this->Paginator->counter(); ?>
		</h6>
	</div>
	<div class="title">
	...
	<table cellpadding="0" cellspacing="0" width="100%" class="sTable mTable">
		<thead>
			<tr>
				...
				<td style="width:15%;">
					<?php echo $this->Paginator->sort("['Pasien'].name",'Nama Pasien');?>
				</td>
				<td style="width:15%;">
					<?php echo $this->Paginator->sort("['LetterType'].name",'Jenis Surat');?>
				</td>
				<?php
				if(
					$access[$aco_id]["_update"] == 1 or
		...
		<tbody>
			...
			<?php foreach($data as $data): ?>
				...
				<tr <?php echo $class?>>
					...
					<td><?php echo chunk_split($data['LetterType']['name'],20,"<br/>") ?></td>
					<?php
					if(
						$access[$aco_id]["_update"] == 1 or
						$access[$aco_id]["_delete"] == 1
