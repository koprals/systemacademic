 ...
 */
    function generate ($data, $settings = array (),$current_selected=array()) {
        ...
        //$view =& ClassRegistry:: getObject('view');//ada error strict standard
        ...
        foreach ($data as $i => $result) {
            ...
            if ($element) {
				...
				if($hasChildren==true)
				{
            		$content = '<input type="checkbox" name="data[Category][][id]" value="'.$result[$model]["id"].'" '.$checked.'/><span class="folder">'.$result[$model][$alias].'</span>';

            	}
            	else
            	{
            		$content = '<input type="checkbox" name="data[Category][][id]" value="'.$result[$model]["id"].'" '.$checked.'/><span class="file">'.$result[$model][$alias].'</span>';
            	}
