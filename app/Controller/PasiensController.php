<?php
class PasiensController extends AppController
{

	//hello world
  var $uses				=	"Pasien";
	var $ControllerName		=	"Pasiens";
	var $ModelName			=	"Pasien";
	...
	function beforeFilter()
	{
		...
		$find					=	$this->MyAco->find("first",array(
										"conditions"	=>	array(
											"LOWER(MyAco.alias)"	=>	strtolower("Pasiens")
										)
									));
  ...
  function Laporan($page=1,$viewpage=50)
	{
		if($this->access[$this->aco_id]["_read"] != "1")
		{
			$this->layout	=	"no_access";
  ...
  function Result($typeListItem = null)
	{
		$this->layout	=	"ajax";
		...
		//DEFINE LAYOUT, LIMIT AND OPERAND
		$viewpage			=	empty($this->params['named']['limit']) ? 50 : $this->params['named']['limit'];
		$order				=	array("{$this->ModelName}.date" => "DESC");
		$operand			=	"AND";
		...
		if(!empty($this->request->data))
		{
			...
			if(!empty($this->request->data['Search']['start_date'])) {
				if(!empty($this->request->data['Search']['end_date'])) {
					$cond_search['Pasien.created between ? and ?'] = array($this->request->data['Search']['start_date'], $this->request->data['Search']['end_date']);
				} else {
					$cond_search['Pasien.created'] = array($this->request->data['Search']['start_date']);
				}
			}
			...
			{
				$this->Session->write("Search.".$this->ControllerName,$cond_search);
				$this->Session->write("Search.".$this->ControllerName."CondSearchTeam", $cond_search_team);
				$this->Session->write('Search.'.$this->ControllerName.'Operand',$operand);
			}
		}
		...
		$ses_cond_search_team	=	$this->Session->read("Search.".$this->ControllerName."CondSearchTeam");
		$cond_search		=	array();
		$filter_paginate	=	array();
		$this->paginate		=	array(
									"{$this->ModelName}"	=>	array(
